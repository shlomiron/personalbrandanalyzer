...
const EMPTY_REPORT = {
  ...
  identityVerification: {
    status: "Ready",
    confidence: 0,
    summary: "Enter any person's name and LinkedIn URL to generate a live public-signal report.",
    limitations: ["No profile has been analyzed yet."],
    sourceNotes: []
  },
  summary: {
    currentPersonalBrand: "Report will appear after analysis.",
  ...
  keyGaps: [{ label: "Generated from entered profile", score: 0, severity: "Live" }],
  swot: { strengths: ["Generated live"], weaknesses: ["Generated live"], opportunities: ["Generated live"], threats: ["Generated live"] },
  priorityActions: [{ action: "Run analysis", supportingPhrase: "Any profile" }],
...
function safeText(value, fallback = "Not enough public evidence found.") { return typeof value === "string" && value.trim() ? value : fallback; }
    ...
    <section className="dashboard">
      <div className="dashboard-head">
        <div>
          ...
          <p>Perception, quantitative metrics, SWOT, and priority actions</p>
        </div>
        <div className="dashboard-icon">📊</div>
      </div>

      <div className="metric-grid">{metrics.slice(0, 5).map((m, i) => <MetricCard key={i} metric={m} />)}</div>
        ...
        <div className="panel accent">
          ...
          {gaps.slice(0, 4).map((g, i) => (
            <div className="gap-row" key={i}>
              <b>{i + 1}</b><span>{g.label}</span>
              <i className="mini"><i style={{ width: scoreWidth(g.score) }} /></i>
              <em>{g.score}/10</em><strong>{g.severity}</strong>
            </div>
          ))}
          <div className="flow">
      ...
      <div className="dash-grid lower">
        <div className="panel">
          ...
          <div className="aud-grid">
            {safeArray(report.audiencePerspective).slice(0, 4).map((a, i) => (
              <div key={i}><b>{a.audience}</b><span>{safeText(a.remembers, "Generated live")}</span></div>
            ))}
          </div>
        </div>
        ...
        <div className="panel">
          ...
          <div className="actions">{actions.slice(0, 3).map((a, i) => <div key={i}><b>{i + 1}</b><span>{a.action} — {a.supportingPhrase}</span></div>)}</div>
        </div>
      </div>
    </section>
  );
...
function Report({ report }) {
  ...
  return (
    <div className="report">
      ...
      {safeArray(iv.limitations).length > 0 && <div className="limitations"><b>Limitations</b>{safeArray(iv.limitations).map((l, i) => <span key={i}>• {l}</span>)}</div>}

      <Dashboard report={report} />

      <Section title="1. First Impression Audit"><div className="two-col">{Object.entries(report.firstImpressionAudit || {}).map(([key, value]) => <div key={key}><b>{key}</b><p>{safeText(value)}</p></div>)}</div></Section>
      <Section title="2. Current Brand Attributes"><div className="table">{safeArray(report.brandAttributes).map((a, i) => <div className="row" key={i}><b>{a.attribute}</b><span>{a.signalType} / {a.strength}</span><p>{a.evidence}</p><em>{a.perception}</em></div>)}</div></Section>
      <Section title="3. Function Analysis"><div className="two-col">{Object.entries(report.functionAnalysis || {}).map(([key, value]) => <div key={key}><b>{key}</b><p>{Array.isArray(value) ? value.join(", ") : safeText(value)}</p></div>)}</div></Section>
      <Section title="4. Form Analysis"><div className="two-col">{Object.entries(report.formAnalysis || {}).map(([key, value]) => <div key={key}><b>{key}</b><p>{Array.isArray(value) ? value.join(", ") : safeText(value)}</p></div>)}</div></Section>
      <Section title="5. Positioning + Consistency"><div className="two-col">{Object.entries(report.positioningConsistency || {}).map(([key, value]) => <div key={key}><b>{key}</b><p>{safeText(value)}</p></div>)}</div></Section>
      <Section title="6. Reputation Gap Table"><div className="gap-table">{safeArray(report.reputationGapTable).map((g, i) => <div key={i}><b>Ideal</b><p>{g.idealPerception}</p><b>Current</b><p>{g.currentPerception}</p><b>Gap</b><p>{g.gap}</p><b>Action</b><p>{g.recommendedAction}</p></div>)}</div></Section>
      <Section title="7. Audience Perspective"><div className="audience-list">{safeArray(report.audiencePerspective).map((a, i) => <article key={i}><h3>{a.audience}</h3><p><b>They think:</b> {a.thinksTheyDo}</p><p><b>Specialty:</b> {a.specialty}</p><p><b>Visual impression:</b> {a.visualImpression}</p><p><b>Trust:</b> {a.trustsFor}</p><p><b>Confusion:</b> {a.confusion}</p><p><b>Memory:</b> {a.remembers}</p></article>)}</div></Section>
      <Section title="8. Brand Statements + Actions"><p><b>Current personal brand:</b> {report.summary?.currentPersonalBrand}</p><p><b>Current form:</b> {report.summary?.currentForm}</p><p><b>Current function:</b> {report.summary?.currentFunction}</p><p><b>Ideal form:</b> {report.summary?.idealForm}</p><p><b>Ideal function:</b> {report.summary?.idealFunction}</p></Section>
      <Section title="Sources">{safeArray(report.sources).length ? <ul className="sources">{safeArray(report.sources).map((s, i) => <li key={i}><a href={s.url} target="_blank" rel="noreferrer">{s.title || s.url}</a> — {s.supports}</li>)}</ul> : <p>No public sources returned yet.</p>}</Section>
    </div>
  );
}
...
function App() {
  ...
  return (
    <main className="shell">
      <section className="hero">
        <div>
          <div className="brand">
            ...
            <div><div className="brand-name">Personal Brand Analyzer</div><div className="brand-kicker">Powered by Visual Storytelling Institute</div></div>
          </div>
      ...
      <section id="results"><Report report={report} /></section>
